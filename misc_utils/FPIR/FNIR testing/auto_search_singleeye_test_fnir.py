...
def top5(probe_template,gallery_array) :
    ...
    for gallery_template in gallery_array :
        ...
        dist_list.append([hamming_distance,i,gallery_template,hamming_distance>0.39])
        ...
        for pic in os.listdir(temp_path2):
            ...
            for i in range(5):
                ...
                if match_result[i][3] :
                    if i == 0:
                        print("Count=",count)
                        print("pcode=",pcode_array[index][1:5])
                        fn_check = True
                ...
                if not match_result[i][3] :
                    if i ==0:
                        print("Count=",count)
                        print("pcode=",pcode_array[index][1:5])
                        if count != pcode_array[index][1:5]:
                            fn_check = True
...
#np.savetxt("single_eye_fnir.csv", data, fmt='%s', delimiter=",", header=columns, comments='')