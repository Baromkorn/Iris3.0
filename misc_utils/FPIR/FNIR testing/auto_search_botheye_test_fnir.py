...
workdir_new = r"C:\Users\USER\Desktop\Open-Iris Dev\Dataset_NECTEC_25072023"
...
for sets in os.listdir(workdir):
    ...
    for side in os.listdir(temp_path1):
        ...
        for pic in os.listdir(temp_path2):
            temp_path3_L = os.path.join(temp_path2,pic)
            temp_pic_L = cv2.imread(temp_path3_L, cv2.IMREAD_GRAYSCALE)
            temp_path3_R = temp_path3_L.replace('L','R')
            if not os.path.exists(temp_path3_R):
                not_path.append(temp_path3_R)
                continue
            temp_pic_R = cv2.imread(temp_path3_R, cv2.IMREAD_GRAYSCALE)
